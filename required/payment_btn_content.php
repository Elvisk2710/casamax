...
if (!mysqli_stmt_prepare($stmt, $sql)) {
    ...
    if ($row = mysqli_fetch_assoc($results)) {
        ...
        $uni =  $row['university'];
        $email = $row['email'];
        $check_sub = "SELECT * FROM subscribers WHERE userid ='$userid' AND completed = 0";
        ...
        if (!$sub_db_check) {
            ...
            if ($rowCount > 0) {
                ...
                $sql = "INSERT INTO subscribers (date_dubscribed,package,number_of_houses,due_date,amount_paid_rtgs userid, university) VALUES ('$this_date','$sub_type','$number_of_houses', '$next_date','$amount','$userid','$uni')";
