...
if(isset($_POST['submit'])){
    ...
    if (empty($tagline) or empty($agent_fee) or empty($firstname) or empty($lastname) or empty($password) or empty($confirmpass) or empty($email) or empty($gender) or empty($contact) or empty($id_num)){
...
if(isset($_POST['register_code'])){
    ...
    if($code == $_COOKIE['code']){
        ...
        $truncated_text = substr($hashedpass, 0, 10);

        $agent_id = $timestamp .'_'.$randomString.'_'.$rand_num.'_'.$trancated_text;
        ...
        if (empty($tagline) or empty($agent_fee) or empty($firstname) or empty($lastname) or empty($password) or empty($confirmpass) or empty($email) or empty($gender) or empty($contact) or empty($id_num)){
            ...
            $sql = "INSERT INTO agents (firstname, lastname, passw, email, sex, contact, id_num,agent_id, agent_fee,agent_tagline ) VALUES ('$firstname', '$lastname', '$hashedpass', '$email', '$gender', '$contact', '$id_num', '$agent_id', '$agent_fee', '$tagline');";
