...
if (isset($_SESSION['sessionagent'])){
    ...
    if(!empty($_POST['borehole'])){
     ...
     if(!empty($_POST['transport'])){
        ...
        $sql = "INSERT INTO homerunhouses (firstname,lastname,price,rules,uni,image1,image2,image3,image4,image5,image6,image7,image8,gender,kitchen,fridge,wifi,borehole,transport,people_in_a_room, agent_id, spots_available, home_location) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if(!mysqli_stmt_prepare($stmt,$sql)){
            ...
            exit();
                ...
                if(!mysqli_stmt_bind_param($stmt,"ssisssssssssssiiiiiisss",$firstname,$lastname,$price,$description,$uni,$image1,$image2,$image3,$image4,$image5,$image6,$image7,$image8,$gender,$kitchen,$fridge,$wifi,$borehole,$transport,$people,$agent_id,$spot,$location)){
                echo '<script type="text/javascript"> alert("SQL ERROR binding stms failed") </script>';
            ...
            if(!mysqli_stmt_execute($stmt)){
                ...
                $new_id = $rand_num . '_'. $agent_id.'_'. $last_id;
                $query_code = "UPDATE homerunhouses	 SET home_id = '" .$new_id. "' WHERE id = '" .$last_id."'";
                $res = mysqli_query($conn,$query_code);
                if(!$res){
                    echo '<script type="text/javascript"> alert("Error while generating ID") </script>';
                    $sql = "DELETE FROM homerunhouses WHERE id = '$last_id'";
                    mysqli_query($conn,$sql);

                }
                if($count <= 0){
                    header("location:./agent_profile.php?error=Profilecreated");
                    echo '<script type="text/javascript"> alert("No images uploaded") 
                    </script>';
                    exit();
