    ...
    if ($password !== $confirmpass) {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($rowCount > 0) {
                ...
                $trancated_text = substr($hashedpass, 0, 10);

                $home_id = $timestamp . '_' . $randomString . '_' . $rand_num . '_' . $trancated_text;

                $sql = "INSERT INTO homerunhouses (home_id,email,firstname,lastname,contact,idnum,price,rules,uni,image1,image2,image3,image4,image5,image6,image7,image8,gender,kitchen,fridge,wifi,borehole,transport,adrs,people_in_a_room,passw) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                if (!$stmt = mysqli_stmt_init($conn)) {
                    ...
                    if (!mysqli_stmt_prepare($stmt, $sql)) {
                        ...
                        if (!mysqli_stmt_bind_param($stmt, "ssssisisssssssssssiiiiisis", $home_id, $email, $firstname, $lastname, $phone, $idnum, $price, $description, $uni, $image1, $image2, $image3, $image4, $image5, $image6, $image7, $image8, $gender, $kitchen, $fridge, $wifi, $borehole, $transport, $address, $people, $hashedpass)) {
