...
session_start();;
        ...
        // if((isset($_SESSION['sessionstudent']) and !empty($_GET['clicked_id']))){
        ...
        //     }
        // }elseif(!empty($_COOKIE['clicked_id']) and !empty($_COOKIE['viewpage'])){
        //     $user  = $_COOKIE['clicked_id'] ;
        //     $sql = "SELECT * FROM  homerunhouses WHERE home_id = '$user'";
        //     setcookie("viewpage", $user, time() + (-86400 * 1), "/");
        //     if ($rs_result = mysqli_query($conn,$sql)){
        //         $row = mysqli_fetch_array($rs_result);
        //         if(!empty($row['agent_id'])){
        //             $agent_id = $row['agent_id'];
        ...
        //     }
        // }else{
        //     header("Location: login.php?error=LogInOrRegister");
...
if ($agent == true){
    ...
    <a href="https://wa.me/+263<?php echo $agent_contact?>?text=Hi%20I%20saw%20your%20listing%20on%20casamax.co.zw%20is%20it%20still%20available.%20https://casamax.co.zw/agent/house.php?home_id=<?php echo $user?>&folder=<?php echo $uni_folder ?>" target='blank'> 
    <button class='contact-host' id='whatsapp'><img src='images/whatsapp.png' alt=''>
    WhatsApp Agent!
    </button>
    </a>

    <a href="tel:263<?php echo $row_agent['contact']?>" target='blank'>
    <button class='contact-host' id='call'><img src='images/dialer.png' alt=''>
    Call Agent!
    </button>
    </a>
...
<?php       
    ...
    echo "
    <a href='https://wa.me/263".$row['contact']."?text=Hello.%20I%20saw%20your%20boarding%20house%20on%20CasaMax.co.zw%20and%20I%20am%20interested%20in%20being%20a%20tenant%20there.%20Is%20it%20still%20available%3F' target='blank'> 
    <button class='contact-host' id='whatsapp'><img src='images/whatsapp.png' alt=''>
    WhatsApp Me!
    </button>
    </a>

    <a href='tel:263".$row['contact']."' target='blank'>
    <button class='contact-host' id='call'><img src='images/dialer.png' alt=''>
    Call Me!
    </button>
    </a>
    ...
    <h2> Address: " . $row['adrs'] . "</h2>
    
      
    <div class='map'>
    <a href='http://maps.google.com/?q=". $row['adrs'] . " Zimbabwe" ."' style='text-decoration: none; animation: text 2s ease infinite; color: rgb(252,153,82);' target='blank'>View in Maps!</a>   
    </div>
